...
_From this point onwards, we are not talking about current production Theme UI._
...
Then, we need to extract the types of keys of our color scale (`primary` and `primary-dark`).
...
type Color =
  | keyof Defined<FinalTheme['colors']>
  | CSS.Globals
```

where `Assign` is a type-level sibling of `Object.assign`, imported from
[_utility-types_](https://github.com/piotrwitek/utility-types) or defined as so
