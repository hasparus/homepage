...
Lines of Code is a lousy metric. At least for on a small scale. I accept
that it may be useful to compare big projects or repositories, but most
often, when I encounter it, it is just meaningless.
...
- My workflow is git-centric. I'm curious how does this repository affects
  their workflow? How is it different from mine?

> The Linux kernel has around 27.8 million lines of code in its Git
> repository

- Linux kernel is way smaller than Google codebase.
- It's still tremendous effort.
...
Can we say that a 500 lines long file is long? Or short? Or is it just
perfect?
...
In a React class component, 150 lines of JSX in a `render` function are
"less code" than 75 lines of class logic.

I will go further. 150 lines of JSX in a functional component are "less
code" than 75 lines of hooks. `useState`, `useEffect`, `useLayoutEffect`,
`useSelector`, `useMachine`. A lot happens there. The difference may not be
so big than when comparing declarative UI composition with imperative code
in lifecycle hooks, but I'd argue that it still holds. Fewer things we have
to comprehend can happen in JSX. (Go away `<Fetch />` component, you're the
outlier.)
