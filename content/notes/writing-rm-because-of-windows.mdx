...
1.  Powershell on it has _[Remove-Item]_ aliased to _rm_.
...
2.  _rm -r -f_ doesn't work too, because _-f_ is ambiguous.
    ...
    All right, for a few months I've been writing _rm -r -fo_ whenever I had
    to remove `node_modules`.

    I aliased<FootnoteA number={1}/> it to _rmrf_ for convenience and with
    the following
